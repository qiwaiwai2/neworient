    ...
    <div class="game">
        ...
        <div class="mapCont">
            <div class="upButton">
                ...
                <a-button type="primary" style="margin-left:10px" @click="saveGPSData">生成卫星数据文件</a-button>
            </div>
            ...
            <div v-else class="noData">该科目暂未导入地图！</div>
        </div>
        <!-- 地图导入/编辑 -->
        <a-modal
            title="导入/编辑地图"
...
export default {
    ...
    props: ['contHeight'],
    mounted () {
        this.cities = this.gameItem[this.game[0]] // 科目列表
        this.gameItemName = this.gameItem[this.game[0]][0] // 科目名称
    ...
    methods: {
        saveGPSData () { // 生成卫星数据文件
            if (this.currentGameId) {
                ...
                this.$message.success('请先选择相应的项目科目组别', 1)
            }
        },
        finesetbtn () { // 精细调整控制
        ...
        handlegameceChange (value) { // 项目选择
            if (value != '请选择') {
                this.cities = this.gameItem[value] // 科目列表
                this.groupList = this.gameItemList[value][this.cities[0]]// 组别列表
                this.gameItemName = this.gameItem[value][0] // 科目名称
                ...
                this.currentGameItemId = this.gameItemListId[value][this.gameItemName] // 科目id
                this.crrrentGroupId = this.gameItemListId[value][this.gameItemName + this.groupName]// 组别id
                this.getMapData()
            }
        },
        handlegameItemceChange (value) { // 科目选择
            ...
            this.currentGameItemId = this.gameItemListId[this.currentGame][value] // 科目id
        ...
        async getGameList () { // 获取项目列表
            ...
            if (data.status == 1) {
                ...
                for (let i = 0; i < gameList.length; i++) {
                    ...
                    this.gameList[ele.name] = ele.id // 项目对应id
                    await findEntity(ele.id).then(res => { // 获取科目列表
                        if (res.status == 1) {
                            let gameDetail = res.data
                            this.gameItem[gameDetail.name] = [] // 项目下的科目
                            this.gameItemList[gameDetail.name] = {} // 组别
                            this.gameItemListId[gameDetail.name] = {} // 科目id
                            gameDetail.itemGroup.forEach((ele, index) => {
                                ...
                                this.gameItemListId[gameDetail.name][ele.itemName] = ele.competitionItemId // 科目id
